...
Spotlight 是一个始终置顶的悬浮窗，拥有多种模式，按 **`Tab`** 键切换，按**`ESC`**退出：
...
*   输入 `=1+1` → 得到结果 2
*   输入 `=sin(pi/2)` → 得到结果 1
*   支持常用数学函数：sin, cos, tan, log, sqrt 等

#### 💻 Shell 命令模式
直接执行终端命令：
*   输入 `>ls -la` (Linux/Mac) 或 `>dir` (Windows)
*   结果会直接显示在 Spotlight 窗口中
*   支持在终端中运行：点击 "Run in terminal" 按钮
...
*   **切换**：在搜索模式下按 `Tab` 键进入紫色界面的 AI 模式，或输入 `/` 打开命令菜单。
*   **对话**：输入问题并回车，体验流式打字机回复。支持 Markdown 渲染和代码高亮。
*   **思考过程**：支持 DeepSeek-R1 等推理模型，可折叠查看 AI 的 "Thinking Process"。
*   **模板 AI**：在提示词管理中勾选 "Use as chat template"，该提示词将作为 AI 对话的系统提示词使用。
...
*   **模板 AI**：在提示词编辑器中勾选 "Use as chat template"，该提示词将作为 Spotlight AI 对话的系统提示词，实现自定义 AI 行为。
...
| `Esc` | 全局 | 关闭当前窗口或弹窗 |
| `/` | Spotlight | 打开命令菜单 (斜杠命令) |
| `=` | Spotlight | 切换到计算器模式 |
| `>` | Spotlight | 切换到 Shell 命令模式 |