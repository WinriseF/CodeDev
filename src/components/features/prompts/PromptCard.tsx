...
interface PromptCardProps {
  ...
  onHoverEnter?: (prompt: Prompt, rect: DOMRect) => void;
  onHoverLeave?: (prompt: Prompt) => void;
}
...
function PromptCardComponent({
  ...
  const handleMouseEnter = () => {
    ...
    if (onHoverEnter && cardRef.current) {
      onHoverEnter(prompt, cardRef.current.getBoundingClientRect());
    }
  };
...
function ActionButton({ icon, onClick, title, danger }: { icon: React.ReactNode; onClick: () => void; title: string; danger?: boolean }) {
  return (
    <button
